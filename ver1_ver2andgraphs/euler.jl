...
function run_synchronization_euler()
    packets_received = 0
    sync_achieved = false
    sync_time = nothing
    start_time = now()
    ...
    println("Slave (Euler): Connecting to master...")
    ...
    while true
        client = try
            ...
            println("Slave (Euler): Connection failed. Retrying...")
            sleep(2)
            continue
        end
        ...
        for j in 1:3
            p1 = plot(t_eval, [master_x[:, j] y_traj[j, :]], 
                    label=["Master" "Slave (Euler)"],
                    xlabel="Time", ylabel="State")
            title!(p1, "Euler Master-Slave Comparison: Component $("xyz"[j])")
            savefig(p1, "euler_comparison_$("xyz"[j]).png")
            
            p2 = plot(t_eval, e_traj[j, :], 
                    label="Error (Euler)",
                    xlabel="Time", ylabel="Error")
            title!(p2, "Euler Error: Component $("xyz"[j])")
            savefig(p2, "euler_error_$("xyz"[j]).png")
        end
        ...
        CSV.write("euler_timing.csv", timing_df)
        ...
        CSV.write("euler_data.csv", df)
        
        break
    end
end

run_synchronization_euler()